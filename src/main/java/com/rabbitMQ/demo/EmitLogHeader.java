...
public class EmitLogHeader {
    ...
    public static void main(String[] argv) {
        ...
        try {
            ...
            if (argv.length < 1) {
                System.err
                        .println("Usage: EmitLogHeader message queueName [headers]...");
                System.exit(1);
            }

            // The API requires a routing key, but in fact if you are using a
            // header exchange the
            // value of the routing key is not used in the routing. You can
            // store information
            // for the receiver here as the routing key is still available in
            ...
            // The rest of the arguments are key value header pairs. For the
            // purpose of this
            // example, we are assuming they are all strings, but that is not
            // required by RabbitMQ
            for (int i = 1; i < argv.length; i++) {
                System.out.println("Adding header " + argv[i] + " with value "
                        + argv[i + 1] + " to Map");
            ...
            // that contains a delivery mode and a priority. So we pass them to
            // the builder.
            builder.deliveryMode(MessageProperties.PERSISTENT_TEXT_PLAIN
                    .getDeliveryMode());
            builder.priority(MessageProperties.PERSISTENT_TEXT_PLAIN
                    .getPriority());
            ...
            channel.basicPublish(EXCHANGE_NAME, routingKey, theProps,
